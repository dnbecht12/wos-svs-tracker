  ...
  return (
    <div className="fade-in">
      ...
      <Section title="Raw Materials" sub="Basic resources — Meat, Wood, Coal, Iron">
        <ResBigInput label="Meat" icon="MT" field="meat" value={inv.meat??0} unit={inv.meatUnit||"M"} onChangeVal={update} onChangeUnit={update} color={COLORS.green} tabIndex={34} />
        <ResBigInput label="Wood" icon="WD" field="wood" value={inv.wood??0} unit={inv.woodUnit||"M"} onChangeVal={update} onChangeUnit={update} color={COLORS.green} tabIndex={35} />
        <ResBigInput label="Coal" icon="CL" field="coal" value={inv.coal??0} unit={inv.coalUnit||"M"} onChangeVal={update} onChangeUnit={update} color={COLORS.green} tabIndex={36} />
        <ResBigInput label="Iron" icon="IR" field="iron" value={inv.iron??0} unit={inv.ironUnit||"M"} onChangeVal={update} onChangeUnit={update} color={COLORS.green} tabIndex={37} />
      </Section>

      <Section title="Other / Misc. Items" sub="Stamina & Speed-ups">
        <ResInput label="Stamina (cans)" icon="🥤" field="stamina" value={inv.stamina??0} onChange={update} color={COLORS.green} tabIndex={38} />
        ...
        <div style={{gridColumn:"1/-1",display:"flex",flexDirection:"column",gap:8,marginTop:4}}>
          ...
          <SpeedupInput label="General"        icon="GN" dField="speedGenD"      hField="speedGenH"      mField="speedGenM"      dVal={inv.speedGenD}      hVal={inv.speedGenH}      mVal={inv.speedGenM}      onChange={update} color={COLORS.blue}   tabIndexBase={39} mode={speedMode} />
          <SpeedupInput label="Troop Training" icon="TR" dField="speedTroopD"    hField="speedTroopH"    mField="speedTroopM"    dVal={inv.speedTroopD}    hVal={inv.speedTroopH}    mVal={inv.speedTroopM}    onChange={update} color={COLORS.green}  tabIndexBase={42} mode={speedMode} />
          <SpeedupInput label="Construction"   icon="CN" dField="speedConstD"    hField="speedConstH"    mField="speedConstM"    dVal={inv.speedConstD}    hVal={inv.speedConstH}    mVal={inv.speedConstM}    onChange={update} color={COLORS.accent} tabIndexBase={45} mode={speedMode} />
          <SpeedupInput label="Research"       icon="RS" dField="speedResearchD" hField="speedResearchH" mField="speedResearchM" dVal={inv.speedResearchD} hVal={inv.speedResearchH} mVal={inv.speedResearchM} onChange={update} color={COLORS.amber}  tabIndexBase={48} mode={speedMode} />
          <SpeedupInput label="Learning"       icon="LN" dField="speedLearningD" hField="speedLearningH" mField="speedLearningM" dVal={inv.speedLearningD} hVal={inv.speedLearningH} mVal={inv.speedLearningM} onChange={update} color={COLORS.blue}   tabIndexBase={51} mode={speedMode} />
          <SpeedupInput label="Healing"        icon="HL" dField="speedHealingD"  hField="speedHealingH"  mField="speedHealingM"  dVal={inv.speedHealingD}  hVal={inv.speedHealingH}  mVal={inv.speedHealingM}  onChange={update} color={COLORS.green}  tabIndexBase={54} mode={speedMode} />
        </div>
        </div>
      </Section>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)",
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14,
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ fontSize:18, fontWeight:800, color:C.textPri,
                fontFamily:"Syne,sans-serif", marginBottom:2 }}>Terms and Conditions</div>
              <div style={{ fontSize:11, color:C.textDim, ...mono }}>
                Last Updated: April 27, 2026 · Effective Date: April 27, 2026
              </div>
            </div>
            <button onClick={onClose} style={{ background:"none", border:"none",
              color:C.textDim, fontSize:20, cursor:"pointer", padding:4 }}>✕</button>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"8px 28px 24px" }}>
          ...
          <P>All Whiteout Survival game names, trademarks, characters, artwork, and game mechanics referenced in this Service are the property of Century Games. Their use here is purely for fan-made, non-commercial reference purposes. We claim no ownership of any such third-party intellectual property. For IP concerns, contact us at deathwishwos@gmail.com.</P>

          <H>7. Privacy and Data</H>
          <P>We collect and store the minimum data necessary to provide the Service, including your email address, authentication tokens, and game planning data you choose to save. We use Supabase (a third-party service provider) to store and sync user data. We do not sell, rent, or trade your personal information to third parties. We may send transactional emails related to your account. We will not send unsolicited marketing emails without your explicit consent.</P>

          <H>8. Payment Terms and Subscriptions</H>
          <P>The Service is currently provided free of charge. We reserve the right to introduce paid subscription tiers in the future. When paid features are introduced: all fees will be displayed clearly before purchase; subscriptions will auto-renew unless cancelled before the renewal date; you may cancel at any time through your account settings; refunds will be issued at our sole discretion except where required by law; and we will provide no less than 14 days advance notice before converting any currently-free feature to a paid feature. All transactions will be processed through a third-party payment processor. We do not store payment card information on our servers.</P>
          ...
          <P>Tundra Command is a fan-made tool. Game mechanics, stats, and formulas in Whiteout Survival may change at any time without notice. We make no guarantee that the Service will reflect such changes in a timely manner.</P>

          <H>10. Limitation of Liability</H>
          <P>TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, TUNDRA COMMAND AND ITS OPERATOR SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED TO LOSS OF PROFITS, DATA, GOODWILL, OR OTHER INTANGIBLE LOSSES, ARISING OUT OF OR IN CONNECTION WITH YOUR USE OF OR INABILITY TO USE THE SERVICE.</P>
          <P>IN NO EVENT SHALL OUR TOTAL LIABILITY TO YOU FOR ALL CLAIMS EXCEED THE GREATER OF: (A) THE AMOUNT YOU PAID TO US IN THE TWELVE MONTHS PRECEDING THE CLAIM, OR (B) ONE HUNDRED DOLLARS (USD $100.00). SOME JURISDICTIONS DO NOT ALLOW THESE LIMITATIONS, SO THEY MAY NOT FULLY APPLY TO YOU.</P>

          <H>11. Indemnification</H>
          <P>You agree to defend, indemnify, and hold harmless Tundra Command and its operator from and against any claims, damages, obligations, losses, liabilities, costs, or expenses (including reasonable attorney's fees) arising from: (a) your use of the Service; (b) your violation of these Terms; (c) your violation of any third-party rights, including intellectual property or privacy rights; or (d) any claim that your User Data caused damage to a third party.</P>

          <H>12. Third-Party Services</H>
          <P>The Service integrates with third-party services including Supabase (database and authentication), Discord (optional OAuth login), and Vercel (hosting and deployment). Your use of these services is governed by their respective terms and privacy policies. We are not responsible for the practices of any third-party services.</P>

          <H>13. Termination</H>
          <P>You may terminate your account at any time using the account deletion feature within the Service, or by contacting us at deathwishwos@gmail.com. We reserve the right to suspend or terminate your access, with or without notice, for any reason including violation of these Terms. Upon termination, your right to use the Service ceases immediately. Sections 6, 9, 10, 11, and 14 survive termination.</P>

          <H>14. Governing Law and Dispute Resolution</H>
          <P>These Terms shall be governed by and construed in accordance with the laws of the State of Florida, United States, without regard to its conflict of law provisions. Any dispute shall first be attempted to be resolved through informal negotiation — please contact us at deathwishwos@gmail.com before initiating formal proceedings. If informal resolution fails, any legal action shall be brought exclusively in the state or federal courts located in Florida, and you consent to the personal jurisdiction of such courts.</P>
          <P>TO THE EXTENT PERMITTED BY LAW, YOU WAIVE ANY RIGHT TO A JURY TRIAL IN CONNECTION WITH ANY ACTION ARISING FROM THESE TERMS. Any claim must be brought within one (1) year of the date the cause of action arose, or it shall be permanently barred.</P>

          <H>15. General Provisions</H>
          <P><strong style={{color:C.textPri}}>Entire Agreement.</strong> These Terms constitute the entire agreement between you and Tundra Command regarding the Service. <strong style={{color:C.textPri}}>Severability.</strong> If any provision is found unenforceable, the remaining provisions continue in full force. <strong style={{color:C.textPri}}>Waiver.</strong> Our failure to enforce any right shall not constitute a waiver. <strong style={{color:C.textPri}}>Assignment.</strong> You may not assign your rights without our prior written consent. We may assign our rights without restriction. <strong style={{color:C.textPri}}>No Partnership.</strong> Nothing in these Terms creates a partnership, joint venture, agency, or employment relationship.</P>
          ...
          <div style={{ marginTop:24, padding:"14px 16px", background:C.surface,
            ...
            <div style={{ fontSize:11, color:C.textDim, ...mono, lineHeight:1.6 }}>
              ...
              Questions about these Terms? Contact us at deathwishwos@gmail.com
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:"14px 28px", borderTop:`1px solid ${C.border}`,
          ...
          <button onClick={onClose} style={{ padding:"8px 24px", borderRadius:7,
            ...
            color:C.textSec, fontSize:12, fontWeight:600 }}>Close</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("admin-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "stat_submissions" }, () => refreshAdminCount())
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "stat_submissions" }, () => refreshAdminCount())
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "issue_reports" }, () => refreshAdminCount())
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "issue_reports" }, () => refreshAdminCount())
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "user_messages" },
        (payload) => { if (payload.new?.sender === "user") refreshAdminCount(); })
      .subscribe();
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`user-realtime-${user.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "user_messages", filter: `user_id=eq.${user.id}` },
        (payload) => { if (payload.new?.sender === "admin") setUserMessages(prev => [...prev, payload.new]); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "user_messages", filter: `user_id=eq.${user.id}` },
        () => fetchUserThreads(user.id).then(setUserMessages))
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "issue_notifications", filter: `user_id=eq.${user.id}` },
        (payload) => { if (payload.new) setNotifications(prev => [...prev, payload.new]); })
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [user]);
  ...
  const handleSwitchCharacter = useCallback(async (newCharId) => {
    ...
    const SYNC_KEYS = [
      ...
      "wos-svs-inventory","wos-rfc-saved-plans","rfc-cycle","rfc-monref","rfc-wdmode","rfc-est-event",
    ];
    SYNC_KEYS.forEach(k => {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      // Table inputs: scope Tab to the nearest table container
      const scope = active.closest("table, .tab-scope");
      if (!scope) return;
      e.preventDefault();
      const fields = Array.from(scope.querySelectorAll(FOCUSABLE)).filter(el => !el.disabled);
